      ...
      <div className="projects">
        {projectsFetched.map((project) => (
          <div key={project.project_id} className="project">
            <h2 style={{ color: "yellow" }}>Project {project.project_name}</h2>
            <p style={{ color: "#f5f5dc" }}>Team Size: {project.team_size}</p>
            <p style={{ color: "#f5f5dc" }}>Workload: {project.workload}</p>
            <p style={{ color: "#f5f5dc" }}>Budget: {project.budget}</p>
            <p style={{ color: "#f5f5dc" }}>
              Est Completion Time: {project.completion_time}{" "}
              {project.completion_time === 1 ? "day" : "days"}
            </p>
